...
namespace RETAIL.BASE.NEG.Interfaces
{
    ...
    public interface ICompanyService : IServiceBase<Company, Company, BaseFilter, int>
    {
        ...
        public Task<ResultModel<bool>> AssignCompanyToUserAsync(int idUser, int idCompany);

        /// <summary>
        /// Removes a company from a user.
        /// </summary>
        /// <param name="idUser">The unique key identifying the user.</param>
        /// <param name="idCompany">The unique key identifying the company.</param>
        /// <returns>A result indicating the success or failure of the operation.</returns>
        public Task<ResultModel<bool>> RemoveCompanyFromUserAsync(int idUser, int idCompany);
    }
}
