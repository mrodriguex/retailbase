...
namespace RETAIL.BASE.API.Controllers.V1
{
    ...
    public class UserController : BaseController
    {
        ...
        public async Task<IActionResult> GetAll([FromQuery] bool? enabled = null, int pageIndex = 1, int pageSize = int.MaxValue)
        {
            ...
            var webResult = await _userservice.GetAllAsync(baseFilter);
            return webResult.Success ? Ok(webResult) : BadRequest(webResult);
        }
        ...
        [HttpDelete("Delete")]
        public async Task<IActionResult> Delete([FromQuery, Required] int idUser)
        {
            var webResult = await _userservice.DeleteAsync(idUser, IdUserAutenticado);
            return webResult.Success ? Ok(webResult) : BadRequest(webResult);
        }
