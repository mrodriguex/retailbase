...
namespace RETAIL.BASE.API.Helpers
{
    /// <summary>
    ...
    public static class JwtAuthenticateHelper
    {
        ...
        /// </returns>
        public static string GenerateJwtToken(int idUser, int tokenDuration, string jwtPrivKey)
        {
            var claims = new[] { new Claim(ClaimTypes.Name, idUser.ToString()) };
        ...
        public static int GetUserIdFromToken(string authHeader)
        {
            ...
            try
            {
                if (authHeader != null && authHeader.StartsWith("Bearer "))
                {
                    ...
                    if (userIdClaim != null)
                    {
                        int.TryParse(userIdClaim.Value, out idUser);
                    }
                }
            }
            catch
